...
exponent(b, 0) // 1 ->base
exponent(b, 1) // b -> base
exponent(b, n) // exponent(b, n / 2) ** 2             [for even n] -> cond
exponent(b, n) // b * (exponent(b, (n - 1) / 2) ** 2) [for odd n] -> cond

You will need to square the results of exponent(b, n / 2) and
(exponent(b, (n - 1) / 2). -> math
...
function advancedExponent(b, n) {
  if (n === 0) return 1
  if (n === 1) return b
  if (!(n % 2)) return advancedExponent(b, n / 2) ** 2
